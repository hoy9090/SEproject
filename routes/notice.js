	...
	pool.getConnection(function(err, conn) {
		...
		if (req.query.search_word == null) {
				...
				if (req.query.pageNo == null) {
					conn.query('select serial_no as no, title, (select nickname from buyer where buyer.serial_no=notice.writter_SN) writter, date_format(date, "%Y-%m-%d") date, views from notice order by serial_no desc limit 0, 10', function(err, result, field) {
						...
						if (count < 5)
							...
							page = [1, 2, 3, 4, 5];
						res.render('notice', {board_title: board_title, category: category, contents: result, page: page, endpage: count, search: false});
					});
				} else {
			...
			conn.query("select count(*) as count from (select title, (select nickname from buyer where buyer.serial_no=notice.writter_SN) writter from notice) as a where "+search_scope+" like ?", ['%'+search_word+'%'], function(err, result, field) {
				...
				var count = parseInt((result[0].count-1)/10)+1;
				if (req.query.pageNo == null) {
					conn.query("select * from (select serial_no as no, title, (select nickname from buyer where buyer.serial_no=notice.writter_SN) writter, date_format(date, '%Y-%m-%d') date, views from notice) as a where "+search_scope+" like ? order by no desc limit 0, 10", ['%'+search_word+'%'], function(err, result, field) {
						if (err)
							console.error(err);
						conn.release();
						var page = [];
						if (count < 5)
							...
							page = [1, 2, 3, 4, 5];
						res.render('notice', {board_title: board_title, category: category, contents: result, page: page, endpage: count, search: true, search_scope: search_scope, search_word: search_word});
					});
				} else {
					var pageNo = parseInt(req.query.pageNo);
					conn.query("select * from (select serial_no as no, title, (select nickname from buyer where buyer.serial_no=notice.writter_SN) writter, date_format(date, '%Y-%m-%d') date, views from notice) as a where "+search_scope+" like ? order by no desc limit ?, 10", ['%'+search_word+'%', (pageNo-1)*10], function(err, result, field) {
						if (err)
							console.error(err);
						conn.release();
						var page = [];
